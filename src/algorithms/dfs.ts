...
export function* dfs(
  ...
  yield {
    ...
    cell: start,
    visitedCount: 0,
    message: 'Initialize: push start onto stack',
    pseudocodeLine: 1,
    queueSnapshot: [start],
  };
  ...
  while (stack.length > 0) {
    ...
    if (r === end[0] && c === end[1]) {
      ...
      for (const cell of path) {
        yield { type: 'path', cell, visitedCount: visited.size, message: 'Path cell', pseudocodeLine: 5 };
      }
      yield { type: 'done', cell: end, visitedCount: visited.size, message: 'Goal reached! (DFS path may not be optimal)' };
      return;
    }
